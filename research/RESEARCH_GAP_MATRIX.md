...
Status: v3, 2026-07-13. Gaps CLOSED by retained execution evidence are listed in §2 with evidence. Open gaps in §1 carry owner + trigger. "Executable" gaps (resolvable only by running code) are deliberately routed to stage workstreams, not desk research — per D-025.
...
| RG-07 | PBO/DSR implementation details vs primary papers (estimator choices, small-sample behavior) | G10 gate correctness | corrected v2 candidate evidence plus completed content-addressed 66-trial campaign (`artifacts/reports/G10_PREREGISTERED_CAMPAIGN_REPORT_2026_07_13.md`): B2/B4 numerically FAIL, B3 is method-blocked; retained implementations are legacy same-close proxies, not canonical next-open conformance | complete upstream search hierarchy, hierarchy-wide dependence/effective trials, canonical conformance evidence if claimed, and stats-specialist review before honoring any future G10 PASS | validation stats specialist | `METHOD_BLOCKED`; immutable family diagnostics retained | blocks full validation/promotion, not offline research | R4/R7 | before any G10 PASS claim |
