"""Vectorbt accelerator for the frozen MVRV-dislocation campaign."""
...
def build_events(
    ...
    *,
    ...
    frame["log_count"] = np.log(frame["mvrv"].astype("float64"))
        ...
        for scenario in request["scenarios"]:
            results[segment][scenario["name"]] = {
                trial_name(item["side"], item["window"], item["holding_days"]): evaluate(
                    spot, mvrv, item, scenario["fee"], scenario["slippage_bps"]
                )
