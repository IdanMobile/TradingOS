"""Independent Decimal ledger for MVRV-dislocation Spot research."""
...
def mvrv_events(
    *,
    ...
    if len(source_days) != len(values) or side not in {"HIGH", "LOW"}:
        raise ValueError("invalid MVRV inputs")
    if window <= 1 or holding_days not in {1, 7} or delay_bars not in {0, 1}:
        raise ValueError("invalid trial parameters")
    if any(not value.is_finite() or value <= 0 for value in values):
        raise ValueError("MVRV values must be positive and finite")
    if any(left >= right for left, right in pairwise(source_days)):
        raise ValueError("MVRV source days must be strictly ordered")
    ...
    for index in range(window, len(source_days)):
        ...
        if len(set(values[index - window : index])) == 1:
            continue
        ...
        if eligible and within_segment:
            candidates.add(source_days[index] + timedelta(days=2, hours=1 + delay_bars))
    source_gap_exits = {
...
def simulate_mvrv_ledger(
    *,
    ...
    return MvrvLedgerResult(tuple(returns), previous_equity, buys, sells, sells, bool(quantity))
