...
from engines.mvrv_data import load_mvrv, load_spot, trial_name  # noqa: E402
...
def event_hash(
    spot: pd.DataFrame, mvrv: pd.DataFrame, trial: dict[str, Any], start: pd.Timestamp
) -> tuple[str, int, int]:
    rows = list(mvrv.itertuples(index=False))
    ...
    for index in range(trial["window"], len(rows)):
        ...
        if any(
            ...
            continue
        baseline = [math.log(item.mvrv) for item in sample[:-1]]
        mean = sum(baseline) / len(baseline)
        variance = sum((value - mean) ** 2 for value in baseline) / len(baseline)
        if variance == 0:
            continue
        z_score = (math.log(sample[-1].mvrv) - mean) / math.sqrt(variance)
...
def main() -> int:
    ...
    mvrv = load_mvrv(ROOT / request["mvrv_path"])
    segments = {}
    for segment, bounds in request["segments"].items():
        ...
        for trial in request["trials"]:
            ...
            digest, buys, sells = event_hash(spot, mvrv, trial, spot.index[0])
