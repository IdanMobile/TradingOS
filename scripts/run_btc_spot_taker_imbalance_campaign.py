"""Run the frozen Spot taker-imbalance campaign behind a select-before-reserve barrier."""
...
from engines.reference.taker_imbalance import (  # noqa: E402
    simulate_taker_ledger,
    taker_events,
)
...
WORKERS = {
    "vectorbt": (
        ...
        ROOT / "engines/vectorbt/taker_imbalance_returns.py",
    ),
    "freqtrade": (
        ROOT / "engines/freqtrade/.venv/bin/python",
        ROOT / "engines/freqtrade/taker_imbalance_signals.py",
    ),
    "nautilus": (
        ROOT / "engines/nautilus/.venv/bin/python",
        ROOT / "engines/nautilus/taker_imbalance_events.py",
    ),
}
...
def preflight(*, require_clean: bool = True) -> dict[str, Any]:
    ...
    if validate(spec_payload).verdict != "VALID":
        raise RuntimeError("canonical taker-imbalance spec is not valid")
    ...
    verification = json.loads(
        subprocess.run(
            [sys.executable, str(ROOT / "scripts/verify_btc_spot_taker_imbalance_data.py")],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
...
def _load_inputs(campaign: Mapping[str, Any]) -> tuple[tuple[Any, ...], tuple[Any, ...]]:
    ...
    spot_data = package["spot_data"]
    early: list[tuple[datetime, Decimal, Decimal, datetime, Decimal, Decimal]] = []
    epoch = datetime(1970, 1, 1, tzinfo=UTC)
    for archive in spot_data["early_archives"]:
        ...
        with zipfile.ZipFile(io.BytesIO(base64.b64decode(encoded))) as zipped:
            ...
            early.extend(
                (
                    epoch + timedelta(milliseconds=int(row[0])),
                    Decimal(row[1]),
                    Decimal(row[4]),
                    epoch + timedelta(milliseconds=int(row[6])),
                    Decimal(row[7]),
                    Decimal(row[10]),
                )
                for row in rows
    ...
    table = pq.read_table(
        ROOT / spot_data["existing_normalized_path"],
        columns=[
            "timestamp_open_utc",
            "open",
            "close",
    ...
    existing = zip(*(table.column(name).to_pylist() for name in table.column_names), strict=True)
    combined = sorted([*early, *existing], key=lambda row: row[0])
    spot = tuple(tuple(row[index] for row in combined) for index in range(3))
    source = (
        tuple(row[0] for row in combined),
        tuple(row[3] for row in combined),
        tuple(row[4] for row in combined),
        tuple(row[5] for row in combined),
    )
    return spot, source
...
def _reference_phase(
    ...
    *,
    ...
    source_opens, source_closes, quote_volume, taker_buy_quote = source
...
def run() -> Path:
    ...
    roster = campaign["trial_roster"]
    temp = Path(tempfile.mkdtemp(prefix=".taker-imbalance-", dir=OUTPUT_ROOT.parent))
    try:
        ...
        selection_path = shared._write_hashed_json(
            ...
            "selection",
            {
                "schema": "tios-btc-spot-taker-imbalance-selection-v1",
        ...
        report = {
            "schema": "tios-btc-spot-taker-imbalance-campaign-result-v1",
