"""Run the frozen transaction-activity campaign behind a select-before-reserve barrier."""
...
CAMPAIGN = ROOT / "research/BTC_TX_ACTIVITY_SPOT_G1_G11_CAMPAIGN_V1.yaml"
OUTPUT_ROOT = ROOT / "artifacts/validation/campaigns/BTC-TX-ACTIVITY-SPOT-G1-G11-V1"
...
WORKERS = {
    "vectorbt": (
        ...
        ROOT / "engines/vectorbt/transaction_activity_returns.py",
    ),
    "freqtrade": (
        ROOT / "engines/freqtrade/.venv/bin/python",
        ROOT / "engines/freqtrade/transaction_activity_signals.py",
    ),
    "nautilus": (
        ROOT / "engines/nautilus/.venv/bin/python",
        ROOT / "engines/nautilus/transaction_activity_events.py",
    ),
}
...
def preflight(*, require_clean: bool = True) -> dict[str, Any]:
    ...
    expected_safety = {
        ...
        "closed_calendar_or_funding_context_access": "PROHIBITED",
    }
    ...
    if validate(spec_payload).verdict != "VALID":
        raise RuntimeError("canonical transaction-activity spec is not valid")
    ...
    verification = json.loads(
        subprocess.run(
            [sys.executable, str(ROOT / "scripts/verify_btc_tx_activity_data.py")],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
...
def _load_inputs(campaign: Mapping[str, Any]) -> tuple[tuple[Any, ...], tuple[Any, ...]]:
    ...
    payload = json.loads((ROOT / campaign["dataset"]["activity_path"]).read_text())
    source_days = tuple(datetime.fromtimestamp(item["x"], UTC) for item in payload["values"])
    counts = tuple(int(item["y"]) for item in payload["values"])
    return spot, (source_days, counts)
    ...
    for segment, bounds in segments.items():
        ...
        for scenario, fee, slippage in SCENARIOS:
            ...
            for trial in trials:
                ...
                entries, exits = activity_events(**kwargs)
                ledger = simulate_activity_ledger(
                    **kwargs,
                    opens=opens,
                    closes=closes,
        ...
        json.dumps(
            {
                ...
                "activity_path": campaign["dataset"]["activity_path"],
                "trials": trials,
                "segments": segments,
                "scenarios": [
...
def _post_selection_reference(
    ...
    return _reference_phase(spot, activity, [selected], POST_SELECTION)
...
def run() -> Path:
    ...
    roster = campaign["trial_roster"]
    temp = Path(tempfile.mkdtemp(prefix=".transaction-activity-", dir=OUTPUT_ROOT.parent))
    try:
        spot, activity = _load_inputs(campaign)
        phase_one, development_returns = _reference_phase(spot, activity, roster, DEVELOPMENT)
        ...
        selection_path = shared._write_hashed_json(
            ...
            "selection",
            {
                "schema": "tios-transaction-activity-selection-v1",
        ...
        phase_two, _ = _post_selection_reference(barrier, spot, activity)
        ...
        delayed_payload, _ = _reference_phase(
            spot, activity, [selected], {"full": POST_SELECTION["full"]}, delay_bars=1
        )
        ...
        report = {
            "schema": "tios-transaction-activity-campaign-result-v1",
            ...
            "closed_calendar_or_funding_context_accessed": False,
            "selection_barrier": {
                "status": "PASS",
