...
- `PaperLaneProposal{proposal_id, strategy_context_ref, mode, gate_ref, requested_synthetic_capital}` is an inert proposal record only. In S2/S3-prep it permits `SYNTHETIC_LOCAL_SIMULATOR` modeling and rejects `VENUE_DEMO_OR_TESTNET` until a later credential gate; all order capabilities remain disabled.
