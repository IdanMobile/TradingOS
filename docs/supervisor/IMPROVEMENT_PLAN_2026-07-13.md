...
| SUP-004 | **COMPLETE** | `make check` enforces controlled-file hashes; v8.52 manifest and changelog are reconciled. |
| SUP-005 | **PARTIAL / METHOD_BLOCKED** | Sharpe is aligned across selection/PBO/DSR; family-scope effective trials use retained correlations where defined; MTF false DSR is removed. Historical upstream hierarchy remains unavailable; the next bounded baseline campaign is fully preregistered before use. |
