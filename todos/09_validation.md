...
- Acceptance: METHOD_VALIDATED verdict or explicit deferral record. Complexity: L. Parallelizable: Yes. Status: **DONE WITH METHOD BLOCK 2026-07-13 — CORRECTED DIAGNOSTICS RETAINED** — method fixtures pass (`G10_METHOD_FIXTURES_2026_07_11.json`), and corrected candidate-specific PBO/CSCV+DSR runs on the retained B2/B3/B4 populations with exact Parquet parity and independent recomputation. B2 numerically FAILS (PBO 0.2960, DSR≈0), B4 numerically FAILS (PBO 0.3810, DSR≈0), and B3 is `METHOD_BLOCKED`; production G10 remains `METHOD_BLOCKED` because the upstream search hierarchy and hierarchy-wide effective-trial evidence are unavailable. Current evidence: `artifacts/validation/G10_CANDIDATE_EVIDENCE_2026_07_13.json`, `scripts/run_g10_candidate.py`, `engines/vectorbt/g10_returns.py`, `tests/test_g10_candidate.py`. The 2026-07-11 artifact is superseded historical evidence. Any future G10 PASS still requires validation-stats-specialist review before promotion use (RG-07 reverify rule).
