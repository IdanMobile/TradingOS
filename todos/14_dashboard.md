...
- T-014-12 global search (FTS): Status **DONE FOR BOUNDED S2 2026-07-11** — Dictionary view projects `DICTIONARY_CONCEPTS_V1` concepts, aliases, contexts, explicit gaps, and FIBO provenance from the dashboard API; `ConceptRegistry` uses SQLite FTS5 and tests cover query behavior. The broader Search view and `GET /api/v1/search` now project concepts, research assets, source records, strategies, and retained reports as a local read-only discovery surface with writes, credentials, venue connection, order endpoint, and execution authority disabled.
- T-014-13 comparisons UI: Status **DONE FOR BOUNDED S2 2026-07-11** — the dashboard Comparison view projects retained candidate dimension scorecards, validation gates, production G10 rows, seed positive-context probes, seed G10, cross-engine scope notes, and evidence refs from local artifacts. It selects no winner and exposes no approval, job, credential, venue, paper/demo/live, or order control.
